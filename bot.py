...
class button_all(Button):
    async def callback(self, interaction):
        c = "Hello " + str(interaction.user)
        c += ", here are the ratings for **" + self.label + "**\n"
        rating = await find_by_id(self.label)
        ratings = ["","",""]
        try:
            ratings[0] = "**" + str(int(100*float(rating['true_opinion']))) + "**"
        except:
            ratings[0] = "**NA**"
        try:
            ratings[1] = "**" + rating['imdb'] + "**"
        except:
            ratings[1] = "**NA**"
        try: 
            ratings[2] = "**" + rating['rt'] + "**"
            ...
            ratings[2] = "**NA**"
        c += "True Opinion: " + ratings[0] + "\n"
        c += "IMDB Rating: " + ratings[1] + "\n"
        c += "Rotten Tomatoes Rating: " + ratings[2] + "\n"
...
class button_imdb(Button):
    async def callback(self, interaction):
        ...
        if float(imdb_rating) > 0:
            ...
            c += "Sorry the IMDB rating for this movie is unavailable, the dev team will be informed."
        await interaction.response.edit_message(content=c, view=None)
...
@bot.slash_command(name='rating',guild_ids=[748940193733804252])
async def rating_result(ctx, query: str):
    await ctx.defer()
    movies_arr = await find_by_id('all_movies')
    algo = SearchAlgorithm(movies_arr['arr'])
    top_5_arr = await algo.top_5(query)
    button1 = button_all(label=top_5_arr[0],style=discord.ButtonStyle.primary,
     emoji="✔")
    button2 = button_all(label=top_5_arr[1],style=discord.ButtonStyle.primary,
     emoji="✔")
    button3 = button_all(label=top_5_arr[2],style=discord.ButtonStyle.primary,
     emoji="✔")
